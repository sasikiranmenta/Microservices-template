...
public class AuthorizationServerConfiguration {
     ...
     * Authentication method – in our case, we'll use basic authentication, which is just a client id and client secret
    ...
    /**
     * Each authorization server needs to have a unique issuer URL as well
     * we will add an “127.0.0.1 auth-server” entry in our /etc/hosts file. This allows us to run the client and the auth server on our local machine, and avoids problems with session cookie overwrites between the two
     */
    @Bean
    public AuthorizationServerSettings authorizationServerSettings() {
        return AuthorizationServerSettings.builder()
                .issuer("http://auth-server:9000") //auth-server is an alias for local-host: https://docs.paperspace.com/core/tutorials/edit-windows-hosts-file/
                .build();
    }
